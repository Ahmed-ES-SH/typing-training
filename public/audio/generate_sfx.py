"""Phase 3 generator: synthesize, master, and validate the 8 SFX WAV files.

Usage:
    python3 generate_sfx.py            # generate + validate + report
    python3 generate_sfx.py --check    # validate existing files only
"""
...
# All SFX render at Tier 3 (-6 dBFS) except the rare x100 overdrive peak at
# Tier 4 (-4 dBFS). Both respect the -3 dBFS master ceiling.
SFX_MANIFEST: list[tuple[str, int, float]] = [
...
def render_sfx(filename: str) -> list[float]:
    """Render a finalized signal for any manifest entry."""
    return sfx_finalize(sfx_sound(filename))
...
def generate_all(root: Path = AUDIO_ROOT) -> list[Path]:
    ...
    written: list[Path] = []
    for filename, _expected_ms, _expected_peak in SFX_MANIFEST:
        output_dir = root / "sfx"
        output_dir.mkdir(parents=True, exist_ok=True)
        signal = render_sfx(filename)
...
def validate_all(root: Path = AUDIO_ROOT) -> tuple[int, int, list[str]]:
    ...
    total = 0
    failures: list[str] = []
    for filename, expected_ms, expected_peak in SFX_MANIFEST:
        total += 1
        path = root / "sfx" / filename
        if not path.is_file():
            failures.append(f"sfx/{filename}: missing file")
            continue
        ...
        if errors:
            failures.append(f"sfx/{filename}: " + "; ".join(errors))
        else:
            passed += 1
...
def print_report(root: Path = AUDIO_ROOT) -> int:
    ...
    print("=" * 70)
    print("           TYPEKERNEL PHASE 3 SFX & COMBO LIBRARY REPORT")
    print("=" * 70)
    print(f"Target Directory:  {root / 'sfx'}")
    print(f"Total Required:    {len(SFX_MANIFEST)} files")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Phase 3 SFX pack")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    if not arguments.check:
        ...
        print(f"Generated {len(written)} sfx files under {AUDIO_ROOT / 'sfx'}")
    return print_report()
